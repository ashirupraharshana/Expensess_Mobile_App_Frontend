...
class SharedPreferencesManager(context: Context) {
    ...
    companion object {
        ...
        private const val KEY_CURRENCY_INDEX = "currency_index"
        ...
        private const val KEY_REMINDER_ENABLED = "reminders_enabled"
        private const val KEY_USER_NAME = "user_name"

        // Default values
        private const val DEFAULT_CURRENCY = "LKR"
        private const val DEFAULT_CURRENCY_INDEX = 0
        ...
        // Currency array for consistent currency handling
    ...
    fun getCurrency(): String {
        // First try to get currency by index (new method)
        val currencyIndex = sharedPreferences.getInt(KEY_CURRENCY_INDEX, -1)
    ...
    fun setCurrencyIndex(index: Int) {
        if (index >= 0 && index < CURRENCIES.size) {
            sharedPreferences.edit().putInt(KEY_CURRENCY_INDEX, index).apply()
            // Also update the currency string for backward compatibility
            setCurrency(CURRENCIES[index])
        }
    }

    fun getCurrencyIndex(): Int {
        return sharedPreferences.getInt(KEY_CURRENCY_INDEX, DEFAULT_CURRENCY_INDEX)
    }
    ...
    fun getCurrentMonthEarnings(): Double {
        val calendar = Calendar.getInstance()
        val currentMonth = calendar.get(Calendar.MONTH)
        val currentYear = calendar.get(Calendar.YEAR)
    ...
    fun getRemainingBudget(): Double {
        val currentExpenses = getCurrentMonthExpenses()
        val budget = getMonthlyBudget()
        return maxOf(0.0, budget - currentExpenses)
    }

    // Get net income for current month (earnings - expenses)
    fun getCurrentMonthNetIncome(): Double {
        return getCurrentMonthEarnings() - getCurrentMonthExpenses()
    }
